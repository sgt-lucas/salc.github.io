...
// para evitar poluir o escopo global, uma prática segura para scripts não-modulares.
(function () {
    document.addEventListener('DOMContentLoaded', () => {
        ...
        function renderPagination(container, totalItems, currentPage, pageSize, onPageChange) {
            ...
            container.innerHTML = `<div class="pagination"><button id="prev-page-btn" class="btn" ${currentPage === 1 ? 'disabled' : ''}><i class="fas fa-chevron-left"></i> Anterior</button><span class="pagination-info">A exibir ${startItem}–${endItem} de ${totalItems}</span><button id="next-page-btn" class="btn" ${currentPage === totalPages ? 'disabled' : ''}>Próxima <i class="fas fa-chevron-right"></i></button></div>`;
        ...
        async function renderDashboardView(container) {
            ...
            container.innerHTML = `<div class="view-header"><h1>Dashboard</h1></div><div class="dashboard-grid"><div class="card kpi-card"><h3>Saldo Disponível Total</h3><p id="kpi-saldo-total">A carregar...</p></div><div class="card kpi-card"><h3>Total Empenhado</h3><p id="kpi-valor-empenhado">A carregar...</p></div><div class="card kpi-card"><h3>NCs Ativas</h3><p id="kpi-ncs-ativas">A carregar...</p></div></div><div class="card aviso-card"><h3><i class="fas fa-exclamation-triangle"></i> Avisos Importantes (Próximos 7 dias)</h3><div id="aviso-content">A carregar...</div></div><div class="card"><div class="view-header"><h3>Gerar Relatório em PDF</h3><button id="generate-report-btn" class="btn btn-primary"><i class="fas fa-file-pdf"></i> Gerar Relatório</button></div><div class="filters"><div class="filter-group"><label for="report-filter-pi">Plano Interno</label><input type="text" id="report-filter-pi" placeholder="Opcional"></div><div class="filter-group"><label for="report-filter-nd">Natureza de Despesa</label><input type="text" id="report-filter-nd" placeholder="Opcional"></div><div class="filter-group"><label for="report-filter-secao">Seção Responsável</label><select id="report-filter-secao"><option value="">Todas</option></select></div><div class="filter-group"><label for="report-filter-status">Status</label><select id="report-filter-status"><option value="">Todos</option><option value="Ativa">Ativa</option><option value="Totalmente Empenhada">Totalmente Empenhada</option></select></div></div><div class="form-field" style="margin-top: 1rem;"><input type="checkbox" id="report-incluir-detalhes" name="incluir-detalhes"><label for="report-incluir-detalhes" style="display: inline-block; margin-left: 0.5rem;">Incluir detalhes de empenhos e recolhimentos</label></div></div>`;
        ...
        async function renderNotasCreditoView(container, page) {
            container.innerHTML = `<div class="view-header"><h1>Gestão de Notas de Crédito</h1><button id="add-nc-btn" class="btn btn-primary"><i class="fas fa-plus"></i> Adicionar Nova NC</button></div><div class="filters card"><div class="filter-group"><label for="filter-pi">Plano Interno</label><input type="text" id="filter-pi" placeholder="Filtrar por PI"></div><div class="filter-group"><label for="filter-nd">Natureza de Despesa</label><input type="text" id="filter-nd" placeholder="Filtrar por ND"></div><div class="filter-group"><label for="filter-secao">Seção Responsável</label><select id="filter-secao"><option value="">Todas</option></select></div><div class="filter-group"><label for="filter-status">Status</label><select id="filter-status"><option value="">Todos</option><option value="Ativa">Ativa</option><option value="Totalmente Empenhada">Totalmente Empenhada</option></select></div><button id="apply-filters-btn" class="btn">Aplicar Filtros</button></div><div class="table-container card"><table id="nc-table"><thead><tr><th>Nº da NC</th><th>Plano Interno</th><th>ND</th><th>Seção</th><th>Valor Original</th><th>Saldo Disponível</th><th>Prazo</th><th>Status</th><th class="actions">Ações</th></tr></thead><tbody></tbody></table></div><div id="nc-pagination-container"></div>`;
        ...
        async function handleNcFormSubmit(e, closeModalFunc) {
            e.preventDefault();
            const form = e.target, submitButton = form.querySelector('button[type="submit"]'), feedbackContainer = form.querySelector('#form-feedback');
        ...
        async function renderEmpenhosView(container, page) {
            container.innerHTML = `<div class="view-header"><h1>Gestão de Empenhos</h1><button id="add-empenho-btn" class="btn btn-primary"><i class="fas fa-plus"></i> Novo Empenho</button></div><div class="table-container card"><table id="empenhos-table"><thead><tr><th>Nº do Empenho</th><th>Nº da NC Associada</th><th>Seção Requisitante</th><th>Valor</th><th>Data</th><th class="actions">Ações</th></tr></thead><tbody></tbody></table></div><div id="empenhos-pagination-container"></div>`;
            ...
            container.querySelector('#add-empenho-btn').addEventListener('click', async () => {
                try {
                    ...
                    openModal('Novo Empenho', formHTML, (modal, close) => {
                        modal.querySelector('#empenho-form').addEventListener('submit', (e) => handleEmpenhoFormSubmit(e, close));
                    });
                } catch (error) {
        ...
        async function renderAdminSeçõesView(container) {
            container.innerHTML = `<div class="card"><h3>Gerir Seções</h3><p>Adicione, renomeie ou exclua seções da lista utilizada nos formulários.</p><form id="secao-form" class="admin-form"><input type="hidden" name="id"><input type="text" name="nome" placeholder="Nome da nova seção" required><button type="submit" class="btn btn-primary">Adicionar Seção</button></form><div class="table-container" style="margin-top: 1.5rem;"><table id="secoes-table"><thead><tr><th>ID</th><th>Nome da Seção</th><th class="actions">Ações</th></tr></thead><tbody></tbody></table></div></div>`;
        ...
        async function handleSecaoFormSubmit(e) {
            e.preventDefault();
            const form = e.target, id = form.id.value, nome = form.nome.value;
        ...
        async function renderAdminUsersView(container) {
            container.innerHTML = `<div class="card"><h3>Gerir Utilizadores</h3><p>Adicione novos utilizadores e defina os seus perfis de acesso.</p><form id="user-form" class="admin-form-grid"><input type="text" name="username" placeholder="Nome de utilizador" required><input type="email" name="email" placeholder="E-mail" required><input type="password" name="password" placeholder="Senha" required><select name="role" required><option value="OPERADOR">Operador</option><option value="ADMINISTRADOR">Administrador</option></select><button type="submit" class="btn btn-primary">Adicionar Utilizador</button></form><div id="form-feedback" class="modal-feedback" style="display: none; margin-top: 1rem;"></div><div class="table-container" style="margin-top: 1.5rem;"><table id="users-table"><thead><tr><th>ID</th><th>Utilizador</th><th>E-mail</th><th>Perfil</th><th class="actions">Ações</th></tr></thead><tbody></tbody></table></div></div>`;
        ...
        async function loadAndRenderUsersTable() {
            ...
            try {
                ...
                tableBody.innerHTML = users.length === 0 ? '<tr><td colspan="5">Nenhum utilizador registado.</td></tr>' : users.map(u => `<tr data-id="${u.id}"><td>${u.id}</td><td>${u.username}</td><td>${u.email}</td><td>${u.role}</td><td class="actions">${u.id === currentUser.id ? '<span>(Você)</span>' : `<button class="btn-icon btn-delete" data-action="delete-user" data-id="${u.id}" data-username="${u.username}" title="Excluir"><i class="fas fa-trash"></i></button>`}</td></tr>`).join('');
            } catch (error) {
        ...
        async function handleUserFormSubmit(e) {
            e.preventDefault();
            const form = e.target, submitButton = form.querySelector('button[type="submit"]'), feedbackContainer = document.querySelector('#form-feedback');
        ...
        async function renderAdminLogsView(container) {
            container.innerHTML = `<div class="card"><h3>Log de Auditoria</h3><p>Registo de todas as ações importantes realizadas no sistema.</p><div class="table-container" style="margin-top: 1.5rem;"><table id="logs-table"><thead><tr><th>Data/Hora (UTC)</th><th>Utilizador</th><th>Ação</th><th>Detalhes</th></tr></thead><tbody></tbody></table></div></div>`;
        ...
        async function loadAndRenderAuditLogsTable() {
            ...
            try {
                ...
                tableBody.innerHTML = logs.length === 0 ? '<tr><td colspan="4">Nenhum log de auditoria encontrado.</td></tr>' : logs.map(log => `<tr><td>${new Date(log.timestamp).toLocaleString('pt-BR', { dateStyle: 'short', timeStyle: 'medium' })}</td><td>${log.username}</td><td><span class="log-action">${log.action}</span></td><td>${log.details || ''}</td></tr>`).join('');
            } catch (error) {
        ...
        appMain.addEventListener('click', async (e) => {
            ...
            if (action === 'add-anulacao') {
                const empenhoId = targetButton.dataset.id;
                const numeroNe = targetButton.dataset.numero;
                ...
                openModal(`Anular Empenho ${numeroNe}`, formHTML, (modal, close) => {
