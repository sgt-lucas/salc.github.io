...
def create_nota_credito(nc_in: NotaCreditoCreate, db: Session = Depends(get_db), current_user: User = Depends(get_current_operator_user)):
    ...
    try:
        # Define o saldo inicial e o status no momento da criação
...
def update_nota_credito(nc_id: int, nc_update: NotaCreditoCreate, db: Session = Depends(get_db), current_user: User = Depends(get_current_operator_user)):
    ...
    saldo_empenhado = db_nc.valor - db_nc.saldo_disponivel
    novo_saldo = nc_update.valor - saldo_empenhado
...
def delete_nota_credito(nc_id: int, db: Session = Depends(get_db), admin_user: User = Depends(get_current_admin_user)):
    ...
    # Regra de negócio: impede a exclusão de NC com empenhos vinculados
    if db_nc.empenhos:
...
def create_empenho(empenho_in: EmpenhoCreate, db: Session = Depends(get_db), current_user: User = Depends(get_current_operator_user)):
    db_nc = db.query(NotaCredito).filter(NotaCredito.id == empenho_in.nota_credito_id).first()
    if not db_nc:
        raise HTTPException(status_code=404, detail="Nota de Crédito não encontrada.")
    ...
    try:
        # A lógica transacional garante que a atualização do saldo e a criação do empenho ocorram juntas ou nenhuma ocorra.
        db_empenho = Empenho(**empenho_in.dict())
        
        # Regra de negócio: Herda a seção da NC pai
        db_empenho.secao_requisitante_id = db_nc.secao_responsavel_id
        
        # Atualiza o saldo e o status da NC
        db_nc.saldo_disponivel -= empenho_in.valor
        if db_nc.saldo_disponivel <= 0.001: # Usar uma pequena tolerância para ponto flutuante
...
def delete_empenho(empenho_id: int, db: Session = Depends(get_db), admin_user: User = Depends(get_current_admin_user)):
    ...
    # Regra de negócio: Não permitir exclusão se houver anulações vinculadas
    anulacoes_count = db.query(AnulacaoEmpenho).filter(AnulacaoEmpenho.empenho_id == empenho_id).count()
    if anulacoes_count > 0:
    ...
    try:
        db_nc = db.query(NotaCredito).filter(NotaCredito.id == db_empenho.nota_credito_id).first()
        
        # Devolve o valor ao saldo da NC e reativa o status se necessário
        if db_nc:
        ...
        create_audit_log(db, admin_user, "EXCLUIR EMPENHO", f"Empenho '{empenho_numero}' (ID: {empenho_id}) foi excluído da NC '{nc_numero}'.")
        db.commit()
        return Response(status_code=status.HTTP_204_NO_CONTENT)
    except Exception as e:
        db.rollback()
...
def create_anulacao(anulacao_in: AnulacaoEmpenhoBase, db: Session = Depends(get_db), current_user: User = Depends(get_current_operator_user)):
    db_empenho = db.query(Empenho).filter(Empenho.id == anulacao_in.empenho_id).first()
    if not db_empenho:
        raise HTTPException(status_code=404, detail="Empenho a ser anulado não encontrado.")
    
    # Valida o valor da anulação
...
def create_recolhimento(recolhimento_in: RecolhimentoSaldoBase, db: Session = Depends(get_db), current_user: User = Depends(get_current_operator_user)):
    db_nc = db.query(NotaCredito).filter(NotaCredito.id == recolhimento_in.nota_credito_id).first()
    if not db_nc:
        raise HTTPException(status_code=404, detail="Nota de Crédito não encontrada.")
    ...
    try:
        ...
        if db_nc.saldo_disponivel <= 0.001:
            db_nc.saldo_disponivel = 0
            # Não muda o status para 'Totalmente Empenhada' em caso de recolhimento, pode-se criar um status 'Recolhida' ou 'Finalizada'
...
def get_dashboard_kpis(db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    # Esta é uma versão simplificada. Filtros seriam adicionados aqui.
    ...
    valor_empenhado = valor_total_original - saldo_total

    return {
        "saldo_disponivel_total": saldo_total,
        "valor_empenhado_total": valor_empenhado,
...
def get_relatorio_pdf(
    db: Session = Depends(get_db), 
    current_user: User = Depends(get_current_user),
    # Aqui viriam os parâmetros de filtro do Query()
):
    buffer = io.BytesIO()
    ...
    # Título dinâmico (exemplo)
    elements.append(Paragraph("RELATÓRIO DE NOTAS DE CRÉDITO", styles['h1']))
    elements.append(Paragraph(f"Gerado por: {current_user.username} em {datetime.now().strftime('%d/%m/%Y %H:%M:%S')}", styles['Normal']))
    elements.append(Spacer(1, 0.25*inch))

    # --- Conteúdo (Exemplo: Relatório Geral de Saldos) ---
    ncs = db.query(NotaCredito).order_by(NotaCredito.plano_interno).all()
    
    table_data = [
        ["PI", "ND", "Nº da NC", "Seção", "Valor Original", "Saldo Disponível", "Status"]
    ]
    for nc in ncs:
        table_data.append([
            nc.plano_interno,
            nc.nd,
            nc.numero_nc,
            nc.secao_responsavel.nome,
            f"R$ {nc.valor:,.2f}",
            f"R$ {nc.saldo_disponivel:,.2f}",
            nc.status
        ])

    table = Table(table_data, colWidths=[1.5*inch, 1*inch, 2*inch, 2*inch, 1.5*inch, 1.5*inch, 1*inch])
    ...
    headers = {'Content-Disposition': 'inline; filename="relatorio_notas_credito.pdf"'}
...
def read_audit_logs(
    ...
    """
    Retorna uma lista paginada de logs de auditoria, dos mais recentes para os mais antigos.
